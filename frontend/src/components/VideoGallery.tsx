...
export const VideoGallery = ({ channelId }: { channelId: string }) => {
  ...
  useEffect(() => {
    ...
    const fetchVideos = async () => {
      ...
      try {
        ...
        const response = await fetch(`https://www.googleapis.com/youtube/v3/search?key=${apiKey}&channelId=${channelId}&part=snippet,id&order=date&maxResults=${videosToLoad}`);
  ...
  const nextVideo = () => {
    setCurrentIndex((prevIndex) => Math.min(prevIndex + videosPerPage, videos.length - videosPerPage));
  };
  ...
  return (
    <Container className="flex flex-col max-w-7xl">
      ...
      <div className="relative w-full overflow-hidden" ref={containerRef}>
        <div className="flex transition-transform duration-300" style={{ transform: `translateX(-${currentIndex * (100 / videosPerPage)}%)` }}>
          {videos.map((video, index) => (
            <div key={index} className={`flex-shrink-0 w-full ${videosPerPage === 1 ? 'sm:w-full' : 'sm:w-1/2'} p-4`}>
              <div className="basic text-lg widget p-8 h-full flex flex-col">
                <h2 className="text-2xl mb-4 line-clamp-2" style={{ minHeight: '3em' }}>{video.title}</h2>
        ...
        <button onClick={prevVideo} className="absolute -left-2.5 top-1/2 transform -translate-y-1/2 basic text-sm p-2" style={{ transform: 'scaleY(3)' }}>
          &lt;
        </button>
        <button onClick={nextVideo} className="absolute -right-2.5 top-1/2 transform -translate-y-1/2 basic text-sm p-2" style={{ transform: 'scaleY(3)' }}>
          &gt;
        </button>
      </div>
    </Container>
